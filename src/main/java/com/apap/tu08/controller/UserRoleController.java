...
public class UserRoleController {
	...
	private String addUserSubmit(@ModelAttribute UserRoleModel user, String password, Model model) {
		if (password.matches("((?=.*\\d)(?=.*[a-zA-Z]).{8,})")) {
			...
			model.addAttribute("user_message", "User berhasil ditambahkan!");
		} else {
			model.addAttribute("user_message", "Password tidak memiliki minimal 8 karakter , atau tidak mengandung kombinasi huruf dan angka");
		}
		return "home";
	}
    		...
    		if (new_password.matches("((?=.*\\d)(?=.*[a-zA-Z]).{8,})")) {
    			if (new_password.equals(conf_password)) {
    				...
    				model.addAttribute("pass_message", "Password berhasil diubah!");
	    				...
	    				model.addAttribute("pass_message", "Silahkan coba lagi. Password dan konfirmasi tidak sama");
	    		}
    		} else {
    			model.addAttribute("pass_message", "Password tidak memiliki minimal 8 karakter , atau tidak mengandung kombinasi huruf dan angka");
    		}
    	} else {
